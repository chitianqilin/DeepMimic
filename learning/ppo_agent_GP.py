...
'''
Proximal Policy Optimization Agent with GP as RNG
'''

class PPOAgentGP(PPOAgent):
    NAME = "PPOGP"
    EPOCHS_KEY = "Epochs"
    ...
    def __init__(self, world, id, json_data): 
        ...
        # KnowStateSequence = [1, 1, 2, 1, 3, 3, 4, 1, 3, 4, 1, 1, 2, 1, 5, 5, 5, 5, 1, 1, \
        #                    3, 3, 1, 4, 4, 3, 3, 1, 4, 1, 1, 1, 1, 2, 1]
        # eng.workspace['KnowStateSequence'] = KnowStateSequence
        self.eng.eval('cd ~/git/oneshot/Codes/KFF_LFM_HMM/Test', nargout=0)
        return

    def GP_train(self, Data_file_path, Configure_file_path):
        self.eng.eval('''KnowStateSequence = [1, 1, 2, 1, 3, 3, 4, 1, 3, 4, 1, 1, 2, 1, 5, 5, 5, 5, 1, 1, ...
                            3, 3, 1, 4, 4, 3, 3, 1, 4, 1, 1, 1, 1, 2, 1]''', nargout=0)
        # PeriodOfEachState = 30
        # eng.workspace['PeriodOfEachState'] = PeriodOfEachState
        self.eng.eval('PeriodOfEachState = 30', nargout=0)
        self.model = self.eng.eval('''model=gphmm_test_argparse('experimentNo', 1000, 'ModelName', 'KFFLFMHMM', ...
    ...
    def GP_sampling():
        self.eng.eval('''[XReturn,YReturn, Yinit, Vinit, PosEnd,VelEnd, ...
    lfXReturn, lfYReturn,funcmeanOut, Kfull] = LFMSampleWithModel(SampleModel, varargin''', nargout=10)
